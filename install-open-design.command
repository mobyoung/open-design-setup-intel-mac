...
# Open Design 桌面启动器
# 功能：通过终端调用 start-open-design.command 启动服务

# 读取配置文件获取安装目录
CONFIG_FILE="$HOME/.open_design_install_dir"
if [[ -f "$CONFIG_FILE" ]]; then
    INSTALL_DIR="$(cat "$CONFIG_FILE" | tr -d '\n')"
    if [[ -n "$INSTALL_DIR" && -f "$INSTALL_DIR/start-open-design.command" ]]; then
        open "$INSTALL_DIR/start-open-design.command"
        exit 0
    fi
fi

# 如果配置文件无效，尝试 .app bundle 检测（兼容旧安装）
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
APPBundle_PATH="$( echo "$SCRIPT_DIR" | grep -o '.*\\.app' | head -1 )"
if [[ -n "$APPBundle_PATH" && "$SCRIPT_DIR" == "$APPBundle_PATH/Contents/MacOS" ]]; then
    INSTALL_DIR="$( dirname "$APPBundle_PATH" )"
    if [[ -f "$INSTALL_DIR/start-open-design.command" ]]; then
        open "$INSTALL_DIR/start-open-design.command"
        exit 0
    fi
fi
...
# 都找不到，报错
osascript -e 'display notification "未找到 Open Design 安装目录或启动脚本！" with title "Open Design 启动失败"' 2>/dev/null
exit 1
LAUNCHER
