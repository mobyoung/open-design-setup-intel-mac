...
**脚本位置说明**：安装完成后，以下脚本会自动生成在 `<open_design_dir>` 内：

- `start-open-design.command`（桌面启动脚本，双击启动）
- `stop-open-design.command`（停止脚本，双击停止服务）
...
**方法 1：双击启动脚本（推荐）**
- 双击 `<open_design_dir>/start-open-design.command`
- 等待 10-15 秒
- 浏览器自动打开 http://127.0.0.1:3001
- macOS 会提示"是否允许运行"，点击"允许"即可

**方法 2：命令行启动**
```bash
cd <open_design_dir>
pnpm tools-dev start desktop --daemon-port 3000 --web-port 3001
```
...
**方法 1：双击停止脚本（推荐）**
- 双击 `<open_design_dir>/stop-open-design.command`
- 等待服务关闭
...
| 日期 | 版本 | 更新内容 |
|------|------|----------|
| 2026-05-08 | 1.1 | 添加 start/stop 脚本使用说明，更新启动方式为 desktop 模式 |
